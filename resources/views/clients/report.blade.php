            ...
            <div class="card shadow">
                <div class="card-header border-0">
                    ...
                    <div class="row align-items-center">
                        <div class="col-6">
                            <h3 class="mb-0">Historial de clientes</h3>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <table class="myTable align-items-center table-bordered  table-hover table-sm" id="clientListDataTable">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th scope="col" class="thTaxationRight">Vehículo</th>
                                    <th scope="col" class="thTaxationMiddle">Fecha compra</th>
                                    <th scope="col" class="thTaxationMiddle">Fecha venta</th>
                                    <th scope="col" class="thTaxationRight">Monto transacción</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($clientList as $key => $value)
                                    @if (!is_null($value->purchases) && count($value->purchases))
                                        @foreach ($value->purchases as $keyPurchase => $valuePurchase)
                                            <tr>
                                                <td>{{ ((int)$value->type === 1) ? "PERSONA" : "EMPRESA" }}</td>
                                                <td>{{ $typeDocumentNames[$value->type_document] }} <b>{{ $value->document_number }}</b></td>
                                                <td>{{ $value->name }}</td>
                                                <td>{{ $valuePurchase->number }}</td>
                                                <td>{{ $valuePurchase->brand }} {{ $valuePurchase->model }} {{ $valuePurchase->color }}</td>
                                                <td>{{ $valuePurchase->created_at }}</td>
                                                <td>--</td>
                                                <td><b>{{ $valuePurchase->currency }} {{ $valuePurchase->price_compra }}</b></td>
                                            </tr>
                                        @endforeach
                                    @endif
                                @endforeach
