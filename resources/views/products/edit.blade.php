			...
			<div class="card shadow">
				<div class="card-header border-0">
					<div class="row align-items-center">
						<div class="col-12">
							<h3 class="mb-0">Actualizar producto - {{ $product->name }}</h3>
						</div>
					</div>
				</div>
				<hr>
				<div class="col-12">
					<form method="post" action="{{ route('products.update', $product->id) }}" autocomplete="off" id="storeForm">
						@csrf
						@method('put')
						<div class="row">
							<div class="col-md-6">
							...
							<div class="col-md-6">
								...
								<div class="form-group">
									...
									<select name="type_product" id="input-type_product" class="form-control ">
										<option {{ (int)$product->type_product === 1 ? "selected": "" }} value="1">PRODUCTO</option>
										<option {{ (int)$product->type_product === 2 ? "selected": "" }} value="2">SERVICIO</option>
									</select>
								</div>
								<div class="row">
									<div class="col-md-6">
								...
								<div class="form-group">
									...
									<select name="flag_active" id="input-flag_active" class="form-control ">
										<option {{ (int)$product->flag_active === 1 ? "selected": "" }} value="1">ACTIVO</option>
										<option {{ (int)$product->flag_active === 0 ? "selected": "" }} value="0">INACTIVO</option>
									</select>
								</div>
							</div>
								...
								<div class="col-md-12">
									<div class="text-center">
										<button type="button" onclick="submitForm('storeForm');" 
											class="btn btn-success mt-4">{{ __('Actualizar producto') }}</button>
									</div>
									<br>
								</div>
							</div>
